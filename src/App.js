...
const App = () => {
	...
	const deleteInputName = (index) => {
		const text = userInputName.split('');
		text.splice(index, 1);
		const updateText = text.join('');
		setUserInputName(updateText);
	};
	const listName = userInputName.split('').map((chars, index) => {
		return <Char character={chars} key={index} clicked={() => deleteInputName(index)} />;
	});
	return (
		<div>
			<input type="Text" onChange={changeInputName} value={userInputName} />
			<p>{userInputName}</p>
